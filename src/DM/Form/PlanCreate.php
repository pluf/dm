 ...
 *
 */
class DM_Form_PlanCreate extends Pluf_Form
{
    ...
    {
        if (! $this->isValid()) {
            throw new \Pluf\Exception('cannot save the plan from an invalid form');
        }
        // Create the plan
        $plan = new DM_Plan();
        
        //Checking existnce of plan template
        $plantemplate = DM_Shortcuts_GetPlanTemplateOr404( $this->cleaned_data ['templateId']);
